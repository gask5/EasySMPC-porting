...
public class InitialMessageManagerEasyBackend extends InitialMessageManager {
    ...
    public List<BusMessage> retrieveMessages() throws IllegalStateException {
        ...
        try {
            resultString = new HTTPRequest(server, PATH_LIST_INITIAL_MESSAGES, HTTPRequest.HTTPRequestType.GET, getToken(), null).execute();
    ...
    protected void deleteMessage(BigInteger id) {
        ...
        try {
            // Delete message
            new HTTPRequest(server, String.format(PATH_DELETE_MESSAGE_PATTERN, id), HTTPRequest.HTTPRequestType.DELETE, getToken(), null).execute();
        } catch (HTTPException e) {
